...
def build_pipeline() -> Pipeline:
    ...
    updated_numerical = NUMERICAL_COLS 
    updated_log_cols = LOG_TRANSFORM_FEATURES_BASELINE
    ...
    column_transformer = ColumnTransformer(
        transformers=[
            ...
            ("log_transform" , FunctionTransformer(np.log1p , validate=True , feature_names_out='one-to-one') , updated_log_cols),
...
@app.command()
def main(
    input_path: Path = PROCESSED_DATA_DIR / "baseline" / "processed_NYC.csv",
    output_path: Path = PROCESSED_DATA_DIR / "baseline" / "features.csv",
    labels_path: Path = PROCESSED_DATA_DIR / "baseline" / "labels.csv",
    test_output_path: Path = PROCESSED_DATA_DIR / "baseline" / "features_test.csv",
    test_labels_path: Path = PROCESSED_DATA_DIR / "baseline" / "labels_test.csv",
    ...
    features_params = params.get("features_baseline", {})
    ...
    transformer_filename = "preprocessor_baseline.joblib"
    try:
