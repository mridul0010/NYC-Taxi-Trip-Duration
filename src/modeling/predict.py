...
from src.config import MODELS_DIR, TARGET_COLUMN, TRAIN_INPUT_PATH , TEST_INPUT_PATH , PROJ_ROOT
...
@app.command()
def main(
    test_features_path: Path = TEST_INPUT_PATH / "features_test.csv",
    test_labels_path: Path = TEST_INPUT_PATH / "labels_test.csv",
    ...
    predictions_path: Path = PROJ_ROOT / "data" / "processed" / "test_predictions.csv",
    metrics_path: Path = PROJ_ROOT / "reports" / "evaluation_metrics.json",
    ...
    try:
        ...
        if use_mlflow:
            model_name = "NYC_Taxi_Trip_Duration_Regressor"
            with mlflow.start_run(run_name=os.getenv("MLFLOW_RUN_NAME", "evaluation")) as run:
                mlflow.set_tag("model", "NYC Taxi Trip Duration Regressor")
